const VISITOR_ID_KEY = "line-up-visitor-id-v2";
const QR_IMAGE_PATH = "/assets/wechat-source.jpg";
...
async function refreshStatus({ silent = false } = {}) {
  try {
    ...
    setInteractionState(true);
    if (!silent) {
      statusText.textContent = "后端服务不可用，请先启动本地服务。";
    }
  }
}
...
function createShareCanvas(ticket) {
  ...
  rows.forEach((row, index) => {
    ...
    context.fillStyle = "#1f2937";
    context.font = '700 34px "Microsoft YaHei", "Segoe UI", sans-serif';
    context.fillText(row[1], x + 28, y + 106);
  });
...
async function shareTicket() {
  ...
  try {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
      ...
      shareStatus.textContent = "取号单已调起分享。";
      return;
    }
    ...
    shareStatus.textContent = "分享图片已下载，可直接发送给别人。";
  } catch (_error) {
...
function cropQrImage() {
  const image = new Image();

  image.addEventListener("load", () => {
    const cropSize = image.width * 0.67;
    const cropX = image.width * 0.165;
    const cropY = image.height * 0.325;
    const canvas = document.createElement("canvas");
